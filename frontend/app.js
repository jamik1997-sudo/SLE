...
function renderWizard(){
  ...
  if(state.step===0)body=questionCards(state.questions.filter(q=>q.step===0),0,map);
    ...
    if(state.step===1){body=`<div class="card"><h2>Торговая точка</h2><div class="field"><label>Код торговой точки / код ТТ</label><div class="input-actions"><input id="shopCode" value="${esc(visit?.shop_code||'')}" placeholder="Введите код ТТ"><button type="button" class="btn secondary" id="scanQr">Сканировать QR</button></div></div></div>`+questionCards(state.questions.filter(q=>q.step===1),state.visit,map)}
...
function visitCheck(){return`<div class="card"><h2>Проверка</h2>${state.audit.visits.map(v=>`<div class="visit-row"><span>Визит ${v.visit_number}: код ТТ ${esc(v.shop_code||'—')}</span><span>Код ТТ: ${esc(v.shop_code||'—')}</span></div>`).join('')}</div>`}
