...
function renderWizard(){
  const meta=stepMeta(),pct=Math.round(meta.screen/37*100),map=answersMap();
  let body='';
  if(state.step===0){
    body=questionCards(state.questions.filter(q=>q.step===0),0,map);
  }else if(state.step===8){
    body=questionCards(state.questions.filter(q=>q.step===8),0,map)+visitCheck();
  }else{
    const visit=state.audit.visits.find(v=>v.visit_number===state.visit);
    if(state.step===1){
      body=`<div class="card"><h2>Торговая точка</h2><div class="field"><label>Код торговой точки / код ТТ</label><input id="shopCode" value="${esc(visit?.shop_code||'')}" placeholder="Введите код ТТ"></div></div><div class="card"><h2>📍 Местоположение</h2><p class="muted">${visit?.latitude!=null?`Координаты сохранены, точность ${Math.round(visit.gps_accuracy||0)} м`:'Координаты сохраняются отдельно для этого визита.'}</p><button class="btn primary full" id="gps">Определить местоположение</button></div>`;
      body+=questionCards(state.questions.filter(q=>q.step===1),state.visit,map);
    }else{
      body=questionCards(state.questions.filter(q=>q.step===state.step),state.visit,map);
    }
    if(state.step===7){
      body+=`<div class="card"><h2>Комментарий</h2><div class="field"><label>Комментарий и рекомендации</label><textarea id="visitComment">${esc(visit?.comment||'')}</textarea></div></div>`;
    }
  }
...
async function adminPage(){
  let regions=[],users=[];
  try{[regions,users]=await Promise.all([api('/admin/regions'),api('/admin/users')])}catch(e){toast(e.message)}
  shell(`<div class="nav"><button class="pill" data-page="home">Главная</button><button class="pill active">Управление</button></div><div class="grid two">${state.me.role==='admin'?`<div class="card"><h2>Добавить регион</h2><form id="addRegion"><div class="field"><label>Название</label><input name="name" required></div><button class="btn primary" style="margin-top:12px">Добавить</button></form></div>`:''}<div class="card"><h2>Добавить сотрудника</h2><form id="addEmployee"><div class="field"><label>ФИО</label><input name="full_name" required></div><div class="field"><label>Регион</label><select name="region_id" required>${regions.map(r=>`<option value="${r.id}">${esc(r.name)}</option>`).join('')}</select></div><div class="field"><label>Должность</label><input name="position"></div><button class="btn primary" style="margin-top:12px">Добавить</button></form></div></div><div class="card"><h2>Создать пользователя</h2><form id="addUser"><div class="grid two"><div class="field"><label>ФИО</label><input name="full_name" required></div><div class="field"><label>Логин</label><input name="login" minlength="3" required></div><div class="field"><label>Пароль</label><input name="password" type="password" minlength="8" placeholder="Минимум 8 символов" required></div><div class="field"><label>Роль</label><select name="role" id="userRole"><option value="leader">Руководитель</option><option value="manager">Менеджер (вся республика)</option>${state.me.role==='admin'?'<option value="admin">Администратор</option>':''}</select></div><div class="field" id="regionField" style="grid-column:1/-1"><label>Регион руководителя</label><select name="region_id" required>${regions.map(r=>`<option value="${r.id}">${esc(r.name)}</option>`).join('')}</select><small class="muted">Руководитель работает только в одном выбранном регионе.</small></div></div><button class="btn primary" style="margin-top:12px">Создать</button></form></div><div class="card"><h2>Пользователи</h2><div class="table-wrap"><table class="table"><tr><th>ФИО</th><th>Логин</th><th>Роль</th><th>Доступ</th></tr>${users.map(u=>`<tr><td>${esc(u.full_name)}</td><td>${esc(u.login)}</td><td>${u.role==='manager'?'Менеджер':u.role==='leader'?'Руководитель':'Администратор'}</td><td>${u.role==='leader'?(u.regions.map(r=>esc(r.name)).join(', ')||'Регион не назначен'):'Вся республика'}</td></tr>`).join('')}</table></div></div>`);
  bindNav();
  const toggleRegion=()=>{const leader=$('#userRole').value==='leader';$('#regionField').hidden=!leader;$('#regionField select').required=leader};
  $('#userRole').onchange=toggleRegion;toggleRegion();
  ...
  $('#addEmployee').onsubmit=async e=>{e.preventDefault();const p=Object.fromEntries(new FormData(e.target));const q=new URLSearchParams(p);try{await api('/admin/employees?'+q,{method:'POST'});toast('Сотрудник добавлен');e.target.reset()}catch(err){toast(err.message)}};
