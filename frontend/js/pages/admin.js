...
async function loadSystemStatus(){
  ...
  try{
    ...
    root.innerHTML=`<div class="site-status-grid">
      ...
      <div class="site-status-item"><span>Пул БД</span>${statusBadge((pool.checked_out??0)<Math.max(1,(pool.size??4)+(pool.overflow??0)),'Норма','Высокая нагрузка')}<small>Активно: ${pool.checked_out??'—'} · пул: ${pool.size??'—'} · overflow: ${pool.overflow??'—'}</small></div>
